...
def init_db():
    ...
    # Ensure standard invitation exists
    cursor.execute("SELECT code FROM invitations WHERE code = 'XHX-F0B2C328'")
    if not cursor.fetchone():
        cursor.execute('''
            INSERT INTO invitations (code, created_by, source, reason, max_uses, current_uses, status)
            VALUES ('XHX-F0B2C328', 'kestrel', 'moltbook', 'Standard Recruitment', 1000, 0, 'active')
        ''')
